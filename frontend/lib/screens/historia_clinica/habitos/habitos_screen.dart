...
import '../shared/widgets/habitos_widget.dart';
...
class _HabitosScreenState extends State<HabitosScreen> {
  ...
  Future<void> _cargarRegistro() async {
    ...
    try {
      // TODO: Implementar endpoint en backend para habitos
      // final response = await _apiService.fetchHabitosById(widget.registroId!);
      // setState(() {
      //   _habitosData = Map<String, dynamic>.from(response['datos']);
      // });
    } catch (e) {
  ...
  void _seleccionarPaciente(Map<String, dynamic> paciente) async {
    ...
    try {
      ...
      if (registros.isNotEmpty) {
        ...
        setState(() {
          _habitosData = Map<String, dynamic>.from(registro['datos']);
