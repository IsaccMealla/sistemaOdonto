...
import '../shared/widgets/antecedentes_periodontal_widget.dart';

class AntecedentesPeriodontalScreen extends StatefulWidget {
  final String? pacienteId;
  final String? historialId;
  final String? estudianteId;
  final String? registroId;

  const AntecedentesPeriodontalScreen({
    Key? key,
    this.pacienteId,
    this.historialId,
    this.estudianteId,
  ...
  @override
  State<AntecedentesPeriodontalScreen> createState() =>
      _AntecedentesPeriodontalScreenState();
}

class _AntecedentesPeriodontalScreenState
    extends State<AntecedentesPeriodontalScreen> {
  ...
  Future<void> _cargarRegistro() async {
    ...
    try {
      // TODO: Implementar endpoint en backend para antecedentes
      // final response = await _apiService.fetchAntecedentesById(widget.registroId!);
      // setState(() {
      //   _antecedentesData = Map<String, dynamic>.from(response['datos']);
      // });
    } catch (e) {
  ...
  Future<void> _guardar() async {
    ...
    try {
      final data = {
        ...
        'materia': 'periodoncia',
        'tipo_registro': 'antecedentes_periodontal',
        'datos': _antecedentesData,
        'estado': 'pendiente',
      };
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: Responsive.isDesktop(context)
          ? null
          : AppBar(
              title: Text('Antecedentes Periodontales'),
                  ...
                  children: [
                    ...
                    if (_selectedPacienteId != null) ...[
                      ...
                      AntecedentesPeriodontalWidget(
                        ...
                        onDataChanged: (data) {
                          setState(() => _antecedentesData = data);
                        },
                        readOnly: false,
                      ),
                      SizedBox(height: 24),
  ...
  void _seleccionarPaciente(Map<String, dynamic> paciente) async {
    ...
    try {
      final registros = await _apiService.fetchRegistrosHistoriaClinica(
        ...
        materia: 'periodoncia',
        tipoRegistro: 'antecedentes_periodontal',
      );
      ...
      if (registros.isNotEmpty) {
        ...
        setState(() {
          _antecedentesData = Map<String, dynamic>.from(registro['datos']);
